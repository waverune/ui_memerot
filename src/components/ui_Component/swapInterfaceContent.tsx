...
const sellAmountsConstruct = (selectedOutputTokens: TokenSymbol[], allocationValues: string[], Amount: bigint) => {
  const sellAmounts = selectedOutputTokens.map((token, index) => {
    ...
    const ratios = allocationValues.map((v) => Number.parseFloat(v) || 0)
    ...
    const tokenShare = (Amount * BigInt(Math.floor(proportion * 10000))) / BigInt(10000)
...
const constructSwapParams = async (
  ...
  const minAmounts = isQuote
    ...
    : quoteResult!.map((amount) => (amount * BigInt(Math.floor((1 - SLIPPAGE_TOLERANCE) * 10000))) / BigInt(10000))
...
function SwapInterfaceContent() {
  ...
  const handleChartIconClick = (coin: string) => {
    console.log(TOKENS[coin].coingeckoId);
    setSelectedChartCoin(coin);
  ...
  const fetchMultiplePriceData = useCallback(
    async (tokensToFetch?: string[]) => {
      try {
        ...
        const newPriceData = results.reduce(
          (acc, result) => {
            if (result.coin_id) {
              acc[result.coin_id] = {
                ...DEFAULT_PRICE_DATA,
  ...
  const getProvider = useCallback(() => {
    ...
    return new ethers.JsonRpcProvider("https://virtual.mainnet.rpc.tenderly.co/87a41065-48e0-49aa-b7cf-593798b729b7")
  }, [publicClient])
  ...
  useEffect(() => {
    if (selectedToken && fromAmount) {
      const balance = Number.parseFloat(tokenBalances[selectedToken] || "0")
      const amountToSell = Number.parseFloat(fromAmount)
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      if (!fromAmount || isNaN(Number.parseFloat(fromAmount))) throw new Error("Invalid input amount")
      console.log(">> paramForSwap", paramForSwap)
      const swapTx = await performSwap(swapContractAddress, selectedToken as string, paramForSwap, signer)
  ...
  const getAllocationForIndex = useCallback(
    (index: number) => {
      ...
      const numericValues = allocationValues.map((v) => Number.parseFloat(v) || 0)
  ...
  const simulateAndLogSwap = useCallback(() => {
    ...
    selectedOutputTokens.forEach((outputToken, index) => {
      ...
      const outputPrice = outputCoinId && tokenPriceData[outputCoinId] ? tokenPriceData[outputCoinId].price_usd : 0
  ...
  const handleTemplateChange = useCallback(
    (newTemplate: string) => {
      ...
      const newToAmounts = Object.fromEntries(
        Object.entries(toAmounts).filter(([token]) => adjustedTokens.includes(token)),
      )
      setToAmounts(newToAmounts)
    },
  ...
  const handleFromAmountChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      const balance = Number.parseFloat(tokenBalances[selectedToken as string] || "0")
      const amountToSell = Number.parseFloat(e.target.value)
  ...
  const handleAllocationTypeChange = (value: "ratio" | "percentage") => {
    ...
    if (value === "ratio") {
      ...
      const percentages = allocationValues.map((v) => Number.parseFloat(v) || 0)
      ...
      const ratios = allocationValues.map((v) => Number.parseFloat(v) || 1)
      ...
      // Check if it matches a template
      const newTemplate = newValues.join(":")
      const isTemplate = ["100", "50:50", "33.33:33.33:33.33", "25:25:25:25", "40:30:30", "50:25:25"].includes(
        newTemplate,
      )
      setSelectedTemplate(isTemplate ? newTemplate : "")
    }
  ...
  const handleAllocationValueChange = (index: number, value: string) => {
    setAllocationValues((prev) => {
      ...
      if (allocationType === "percentage") {
        ...
        const total = newValues.reduce((sum, v) => sum + (Number.parseFloat(v) || 0), 0)
        if (total > 100) {
  ...
  const getAllocationString = () => {
    if (allocationType === "ratio") {
      ...
      const values = allocationValues.map((v) => Number.parseFloat(v) || 0)
  ...
  const simulateQuote = useCallback(async () => {
    ...
    try {
      ...
      selectedOutputTokens.forEach((token, index) => {
        ...
        const amount = ethers.formatUnits(quoteResult[index] || 0, TOKENS[token].decimals)

        // Get input token price
        const inputCoinId = TOKENS[selectedToken].coingeckoId
        const inputPrice = inputCoinId && tokenPriceData[inputCoinId] ? tokenPriceData[inputCoinId].price_usd : 0
        ...
        const allocatedInputAmount = (Number.parseFloat(fromAmount) * allocation).toString()

        // Calculate price impact
        const priceImpact = calculatePriceImpact(allocatedInputAmount, amount, inputPrice, outputPrice)

        // Calculate USD value
        const usdValue = (Number.parseFloat(amount) * outputPrice).toString()
  ...
  return (
    <div className="relative">
      ...
      <div className="absolute left-8 top-1/2 -translate-y-1/2 flex flex-col gap-8">
        {Object.keys(TOKENS).slice(0, 3).map((coin) => (
          <button
            key={coin}
            onClick={() => handleChartIconClick(coin)}
            className="p-3 rounded-lg hover:bg-blue-800 transition-colors"
          >
            <img
              src={TOKENS[coin].logo}
              alt={TOKENS[coin].symbol}
              width={40}
              height={40}
      ...
      <div className="absolute right-8 top-1/2 -translate-y-1/2 flex flex-col gap-8">
        {Object.keys(TOKENS).slice(-3).map((coin) => (
          <button
            key={coin}
            onClick={() => handleChartIconClick(coin)}
            className="p-3 rounded-lg hover:bg-blue-800 transition-colors"
      ...
      {/* Existing swap interface content */}
      <div className="relative z-10 mx-16">
        {/* Main content area with fixed height and scrolling */}
        <div className="swap-container  flex flex-col ">
          <div className="swap-content flex-1">
            <div className="swap-wrapper max-w-7xl mx-auto my-2">
                        ...
                        <div className="flex flex-col items-end">
                          ...
                          >
                            {selectedToken ? (
                              <>
                                <img
                                  src={TOKENS[selectedToken].logo || "/placeholder.svg"}
                          ...
                          <span className="text-xs text-gray-400 mt-1">
                            Balance: {Number.parseFloat(tokenBalances[selectedToken as string] || "0").toFixed(4)}{" "}
                            {selectedToken}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div key={index} className="bg-gray-700 rounded-lg p-3 flex flex-col">
                          <div className="flex items-center justify-between">
                            <div className="flex-grow">
                              ...
                              <div className="flex flex-col space-y-1">
                                ...
                                {selectedOutputTokens[index] &&
                                  simulatedOutputs[selectedOutputTokens[index]]?.priceImpact && (
                                    <span
                                      className={`text-xs ${
                                        Number.parseFloat(
                                          simulatedOutputs[selectedOutputTokens[index]]?.priceImpact || "0",
                                        ) > 5
                                          ? "text-red-400"
                                          : "text-green-400"
                                      }`}
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              >
                                {selectedOutputTokens[index] && selectedOutputTokens[index] !== "" ? (
                                  <>
                                    <img
                                      src={TOKENS[selectedOutputTokens[index]].logo || "/placeholder.svg"}
                          ...
                          {selectedOutputTokens[index] && selectedOutputTokens[index] !== "" && (
                            <div className="text-xs text-gray-400 mt-1 text-right">
                              Doge ratio: {calculateDogeRatio(selectedOutputTokens[index], tokenPriceData, dogeMarketCap)}x
                              Balance:{" "}
                              {Number.parseFloat(tokenBalances[selectedOutputTokens[index] as string] || "0").toFixed(2)}
                            </div>
                          )}
                        </div>
                      ))}
                      ...
                      {getSelectedTokenCount() < 4 && (
                        ...
                        >
                          ...
                          Add another token
                          <span className="text-xs text-gray-400 ml-2">({4 - getSelectedTokenCount()} remaining)</span>
                        </button>
                      )}
                    </div>
                  </div>
                ...
                <div className="w-full lg:w-1/2 space-y-4 mt-8 lg:mt-0">
                  <div className="bg-gray-800 rounded-lg p-4 space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-sm text-gray-400">Current Allocation: {getAllocationString()}</div>
                    </div>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="relative w-full h-4 bg-gray-700 rounded">
                          {allocationPercentages.map((percentage, index) => {
                            const leftPosition = allocationPercentages.slice(0, index).reduce((a, b) => a + b, 0) // Calculate left position for the segment
        ...
        <ChartModal
          ...
          selectedCoin={selectedChartCoin}
        />
      </div>
    </div>
  )
