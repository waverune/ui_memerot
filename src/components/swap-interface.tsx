...
import { ArrowDownUp, ChevronDown, Info, Lock, Unlock, Plus } from 'lucide-react'
...
export function SwapInterface() {
  ...
  const [toAmounts, setToAmounts] = useState<Record<Token, string>>({ SPX6900: '2000', MOG: '89213.4' })
  const [sliderValues, setSliderValues] = useState<Record<Token, number>>({ SPX6900: 50, MOG: 50 })
  const [lockedTokens, setLockedTokens] = useState<Record<Token, boolean>>({ SPX6900: false, MOG: false })
  ...
  const handleSliderChange = (token: Token, value: number) => {
    ...
    const unlockedTokens = Object.keys(newSliderValues).filter(t => !lockedTokens[t]);
    ...
    setToAmounts(prev => 
      Object.fromEntries(
        Object.entries(sliderValues).map(([token, value]) => [
          token,
          ((value / 100) * totalAmount / (token === 'SPX6900' ? spx6900Price : mogPrice)).toFixed(2)
        ])
      )
    );
      ...
      <div className="flex-grow flex items-center justify-center">
        <div className="w-full max-w-md space-y-4">
          <div className="flex items-center space-x-2 bg-gray-800 rounded-lg p-2">
            <Switch id="simple-swap" checked={simpleSwap} onCheckedChange={setSimpleSwap} />
            <label htmlFor="simple-swap" className="text-sm">
              Try Simple Swap - No Slippage or Gas Fees
            </label>
            <span className="bg-orange-500 text-xs font-bold px-2 py-1 rounded">NEW</span>
          </div>
              ...
              <div className="bg-gray-700 rounded-lg p-3 flex justify-between items-center">
                <div className="flex flex-col items-start space-y-1">
                  <div className="flex items-center space-x-2">
                    <div className="w-8 h-8 bg-gray-600 rounded-full flex items-center justify-center">ETH</div>
                ...
                <div key={token} className="space-y-2">
                  <div className="bg-gray-700 rounded-lg p-3 flex justify-between items-center">
                    <div className="flex flex-col items-start space-y-1">
                      <div className="flex items-center space-x-2">
                        <Image
                          src={token === 'MOG' ? mogLogo : (token === 'SPX6900' ? spx6900Logo : '/placeholder.svg')}
                          alt={token}
                          width={32}
                          height={32}
                    ...
                    <div className="flex flex-col items-end">
                      <Input
                        type="number"
                        value={amount}
                        onChange={(e) => setToAmounts(prev => ({ ...prev, [token]: e.target.value }))}
                        className="bg-transparent border-none text-right w-24"
                      />
                      <span className="text-xs text-gray-400">
                        ${(parseFloat(amount) * (token === 'SPX6900' ? spx6900Price : mogPrice)).toFixed(2)}
                      </span>
                    </div>
                  </div>
